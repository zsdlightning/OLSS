...
''' on conte.rcac.purdue.edu:/home/szhe/online-ss/ss '''
def test_news20(rho0):
    training_file = '../../data/news20/news20.binary.shuf.vw.train'
    testing_file = '../../data/news20/news20.binary.shuf.vw.test'
    ...
    save_sparse_csr('./news20.test.X', Xtest)
    np.save('./news20.test.y', ytest)
    sys.exit(1)
    '''

    Xtest = load_sparse_csr('./news20.test.X.npz')
    ytest = np.load('./news20.test.y.npy')
    ...
    ep = EPSS(d, rho0 = rho0, n_epoch = 1, mini_batch = 100, tol = 1e-5, damping = 0.9, tau0 = 5000.0)
    with open('logger-news20.txt','w') as f:
...
if __name__ == '__main__':
    np.random.seed(0)
    if len(sys.argv) != 2:
        print 'Usage: %s <rh0>'%sys.argv[0]
        sys.exit(1)

    test_news20(float(sys.argv[1]))
