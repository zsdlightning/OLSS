...
class EPSS:
    ...
    def train_stochastic_multi_rate(self, d, n_pos, n_neg, training_file, fea2id, fea2stats, Xtest, ytest, logger, n_batch_update_prior = 1, intercept = False, damping_both = True):
        ...
        while it < self.n_epoch:
            ...
            pred_label[pred>0] = 1
            pred_label[pred<=-0] = -1
            f1 = metrics.f1_score(ytest, pred_label, pos_label=1)
            print >>logger, 'epoch %d, rho0 = %g,  auc = %g,f1 = %g, feature # = %d'%(it, expit(self.rho), val, f1, np.sum(self.r>0))
            print 'epoch %d, rho0 = %g,  auc = %g,f1 = %g, feature # = %d'%(it, expit(self.rho), val, f1, np.sum(self.r>0))
            np.save('model.epoch%d.%s'%(it, os.path.basename(sys.argv[0])), self.mu*(self.r>0))
            np.save('sel.epoch%d.%s'%(it, os.path.basename(sys.argv[0])), expit(self.r))
            it = it + 1
            curr = 0
...
def test_rcv1(rho0):
    training_file = '../../data/rcv1/rcv1.train.vw'
    testing_file = '../../data/rcv1/rcv1.test.vw'
    fea2id = load_feature_id('feature_to_id.txt')
    ...
    save_sparse_csr('../../data/rcv1/rcv1.test-X', Xtest)
    np.save('../../data/rcv1/rcv1.test-y', ytest)
    sys.exit(1)
    '''

    Xtest = load_sparse_csr('../../data/rcv1/rcv1.test-X.npz')
    ytest = np.load('../../data/rcv1/rcv1.test-y.npy')

    d = len(fea2id)
    #calc_feature_appearence_separately(d, fea2id, training_file)
    n_pos = np.load('feature_appearence_pos.npy')
    n_neg = np.load('feature_appearence_neg.npy')
    #tau0=1000 is the best choice
    ep = EPSS(d, rho0 = rho0, n_epoch = 1, mini_batch = 100, tol = 1e-5, damping = 0.9, tau0 = 1000.0)
    with open('logger-rcv1.txt','w') as f:
    ...
    w = ep.mu * ((ep.r)>0.5)
    np.save('model-rcv1.npy', w)
    r = ep.r
    np.save('sel-rcv1.npy', r)



if __name__ == '__main__':
    if len(sys.argv) != 2:
        print 'usage %s <rho0>'%sys.argv[0]
        sys.exit(1)

    np.random.seed(0)
    test_rcv1(float(sys.argv[1]))
